...
from typing import Annotated, Any, ForwardRef, TypeAliasType, Union, get_args, get_origin, get_type_hints, cast
...
# ==========================================
FORBIDDEN_CRUD = ["Data", "Model", "Type", "Info", "ID", "Record", "Create", "Read", "Update", "Delete", "Remove", "Group", "List", "Memory", "Link", "Merge", "Overwrite", "History"]
REQUIRED_SUFFIXES = ["Receipt", "Event", "Premise", "Intent", "Task", "Policy", "Contract", "SLA", "State", "Snapshot", "Manifest", "Profile", "Proxy", "Mask", "Constraint", "Invariant"]
        ...
        class CodeVisitor(ast.NodeVisitor):
            def visit_ClassDef(self, node: ast.ClassDef) -> None:
                ...
                if not name.startswith("_"):
                    ...
                    errors.extend(f"Forbidden CRUD term '{crud}' in name" for crud in FORBIDDEN_CRUD if crud.lower() in name.lower() and not is_exception and crud in name)
                    ...
                    if inherits_base:
                        ...
                        if not docstring:
                            ...
                            errors.extend(f"Missing '{part}' in docstring" for part in DOCSTRING_PARTS if part not in docstring)
                if errors:
...
def evaluate_architectural_manifold() -> None:
    FORBIDDEN_PATTERNS = [r"import\s+fastapi", r"from\s+fastapi\s+import", r"import\s+flask\b", r"from\s+flask\s+import", r"os\.mkdir", r"os\.makedirs", r"logger\.add"]
...
def evaluate_ast_instantiation_bounds() -> None:
    def is_forbidden_config(node: ast.expr) -> bool:
        ...
        if isinstance(node, ast.Call) and (getattr(node.func, "id", None) == "ConfigDict" or getattr(node.func, "attr", None) == "ConfigDict"):
            for kw in node.keywords:
                if kw.arg in forbidden_keys and not (isinstance(kw.value, ast.Constant) and kw.value.value is True):
                    return True
        elif isinstance(node, ast.Dict):
            for key, value in zip(node.keys, node.values, strict=False):
                if isinstance(key, ast.Constant) and key.value in forbidden_keys and not (isinstance(value, ast.Constant) and value.value is True):
                    return True
        return False
    ...
    def get_decorators(node: ast.FunctionDef) -> set[str]:
        ...
        for dec in node.decorator_list:
            if isinstance(dec, ast.Name): decorators.add(dec.id)
            elif isinstance(dec, ast.Call) and isinstance(dec.func, ast.Name): decorators.add(dec.func.id)
            elif isinstance(dec, ast.Attribute): decorators.add(dec.attr)
            elif isinstance(dec, ast.Call) and isinstance(dec.func, ast.Attribute): decorators.add(dec.func.attr)
        return decorators
                ...
                for node in ast.walk(tree):
                    if isinstance(node, ast.ClassDef):
                        ...
                        for base in node.bases:
                            if isinstance(base, ast.Name): bases.add(base.id)
                            elif isinstance(base, ast.Attribute): bases.add(base.attr)
                            elif isinstance(base, ast.Subscript) and isinstance(base.value, ast.Name): bases.add(base.value.id)
    ...
    for filepath in py_files:
        with open(filepath, encoding="utf-8") as f:
            source = f.read()
            try: tree = ast.parse(source, filename=str(filepath))
            except SyntaxError as e:
                print(f"Syntax error in {filepath}: {e}", file=sys.stderr)
                has_violations = True; continue
        ...
        def is_coreason_model(class_name: str, visited: frozenset[str] = frozenset()) -> bool:
            if class_name == "CoreasonBaseState": return True
            if class_name in visited or class_name not in known_classes: return False
            return any(is_coreason_model(base, visited | frozenset([class_name])) for base in known_classes[class_name])

        for node in ast.walk(tree):
            if isinstance(node, ast.ClassDef):
                if not is_coreason_model(node.name): continue
                for item in node.body:
                    if isinstance(item, ast.FunctionDef):
                        if item.name in ("__init__", "__post_init__"):
                            print(f"Rule A Violation: Class '{node.name}' defines forbidden method '{item.name}' in {filepath}", file=sys.stderr)
                            has_violations = True
                        if item.name not in allowed_methods:
                            ...
                            if "model_validator" not in decorators and "field_validator" not in decorators:
                                print(f"Rule B Violation: Class '{node.name}' function '{item.name}' missing validator decorator in {filepath}", file=sys.stderr)
                                has_violations = True
                    elif isinstance(item, ast.AnnAssign):
                        if isinstance(item.target, ast.Name) and item.target.id == "model_config" and item.value is not None and is_forbidden_config(item.value):
                            print(f"Rule C Violation: Class '{node.name}' attempts to bypass immutability lock in {filepath}", file=sys.stderr)
                            has_violations = True
                    elif isinstance(item, ast.Assign):
                        for target in item.targets:
                            if isinstance(target, ast.Name) and target.id == "model_config" and is_forbidden_config(item.value):
                                print(f"Rule C Violation: Class '{node.name}' attempts to bypass immutability lock in {filepath}", file=sys.stderr)
                                has_violations = True
    if has_violations:
...
def evaluate_topological_reachability() -> None:
    ...
    CLASS_REGISTRY = {cls.__name__: cls for cls in get_all_subclasses(onto.CoreasonBaseState) if cls.__name__ not in EXCLUDED_BASE_CLASSES}
    ...
    def extract_referenced_models(annotation: Any, seen: set[int | str] | None = None) -> list[type]:
        ...
        if ann_id in seen: return []
        seen.add(ann_id)
        if isinstance(annotation, str):
            clean_string = annotation.strip("'\"")
            if clean_string in ALIAS_REGISTRY: return extract_referenced_models(ALIAS_REGISTRY[clean_string], seen)
            if clean_string in CLASS_REGISTRY: return [CLASS_REGISTRY[clean_string]]
            return []
        if isinstance(annotation, ForwardRef): return extract_referenced_models(annotation.__forward_arg__, seen)
        if isinstance(annotation, TypeAliasType): return extract_referenced_models(annotation.__value__, seen)
        origin = get_origin(annotation)
        if origin is Annotated:
            args = get_args(annotation)
            if args: return extract_referenced_models(args[0], seen)
            return []
        if origin is Union or origin is types.UnionType:
            result = []
            for arg in get_args(annotation): result.extend(extract_referenced_models(arg, seen))
            return result
        ...
        try:
            ...
            for resolved_type in hints.values():
                for ref_model in extract_referenced_models(resolved_type):
                    if ref_model.__name__ in CLASS_REGISTRY: G.add_edge(cls_name, ref_model.__name__)
        except Exception as e:
            print(f"Warning: Failed to resolve type hints for {cls_name}: {e}")

    ROOT_NODES = [
        "WorkflowManifest", "EpistemicLedgerState", "StateHydrationManifest", "KinematicDeltaManifest", "TraceExportManifest",
        "FederatedSecurityMacroManifest", "CognitiveSwarmDeploymentManifest", "AdversarialMarketTopologyManifest",
        "ConsensusFederationTopologyManifest", "CapabilityForgeTopologyManifest", "IntentElicitationTopologyManifest",
        "NeurosymbolicVerificationTopologyManifest", "DAGTopologyManifest", "CouncilTopologyManifest", "SwarmTopologyManifest",
        "EvolutionaryTopologyManifest", "SMPCTopologyManifest", "EvaluatorOptimizerTopologyManifest", "DigitalTwinTopologyManifest",
        "DiscourseTreeManifest", "OntologicalSurfaceProjectionManifest", "FederatedDiscoveryManifest", "PresentationManifest",
        "DynamicManifoldProjectionManifest", "MCPClientIntent", "OntologyDiscoveryIntent", "SemanticMappingHeuristicIntent",
        "TerminalCognitiveEvent", "CognitiveActionSpaceManifest", "EpistemicSOPManifest", "EpistemicDomainGraphManifest",
        "EpistemicTopologicalProofManifest", "EpistemicCurriculumManifest", "ExecutionEnvelopeState", "JSONRPCErrorResponseState",
        "JSONRPCErrorState", "CrossSwarmHandshakeState", "OntologicalHandshakeReceipt", "StateDifferentialManifest",
        "ComputeEngineProfile", "DelegatedCapabilityManifest", "ComputationalThermodynamicsProfile", "ActiveInferenceEpochState",
        "AuctionState", "AdversarialSimulationProfile", "ChaosExperimentTask", "DocumentLayoutManifest", "DynamicLayoutManifest",
        "AdjudicationRubricProfile", "CognitiveSamplingPolicy", "ContinuousMutationPolicy", "DifferentiableLogicPolicy",
        "DistributionProfile", "EnsembleTopologyProfile", "EpistemicEscalationContract", "EpistemicSeedInjectionPolicy",
        "FederatedPeftContract", "GovernancePolicy", "GradingCriterionProfile", "GraphFlatteningPolicy", "KineticBudgetPolicy",
        "MCPPromptReferenceState", "MCPResourceManifest", "MarketContract", "ReasoningEngineeringPolicy", "SemanticEdgeState",
        "SpatialReferenceFrameManifest", "TaxonomicRoutingPolicy", "UtilityJustificationGraphReceipt", "ViewportProjectionContract",
        "EpistemicQuarantineSnapshot", "MultimodalArtifactReceipt", "DynamicRoutingManifest", "EpistemicChainGraphState",
        "EpistemicTransmutationTask", "EpistemicUpsamplingTask", "SyntheticGenerationProfile", "NDimensionalTensorManifest"
    ]
    reachable_nodes = set(ROOT_NODES)
    for root in ROOT_NODES:
        if root in G: reachable_nodes.update(nx.descendants(G, root))
    orphaned_nodes = set(G.nodes) - reachable_nodes
    if len(orphaned_nodes) > 0:
        print("CRITICAL FAULT: True Orphaned Nodes Detected")
        print("-" * 50)
        for node in sorted(orphaned_nodes): print(node)
        print("-" * 50)
        sys.exit(1)
    else:
...
def calculate_semantic_differential() -> None:
    def get_head_schema() -> dict[str, Any]:
        try:
            output = subprocess.check_output(["git", "show", "HEAD~1:coreason_ontology.schema.json"], stderr=subprocess.DEVNULL, text=True) # noqa: S607
            return cast("dict[str, Any]", json.loads(output))
        except subprocess.CalledProcessError: return {}
    def get_current_schema() -> dict[str, Any]:
        path = REPO_ROOT / "coreason_ontology.schema.json"
        if not path.exists(): return {}
        with open(path, encoding="utf-8") as f: return cast("dict[str, Any]", json.load(f))
    ...
    for name, old_def in old_defs.items():
        if name not in new_defs: continue
        new_def = new_defs[name]
        added_required = set(new_def.get("required", [])) - set(old_def.get("required", []))
        if added_required: breaking_changes.append(f"[{name}] Contravariance Violation: added required properties: {added_required}")
        ...
        for prop_name, old_prop in old_props.items():
            if prop_name in new_props:
                if old_prop.get("type") != new_props[prop_name].get("type"):
                    breaking_changes.append(f"[{name}.{prop_name}] Covariance Violation: type changed from {old_prop.get('type')} to {new_props[prop_name].get('type')}")
    if breaking_changes:
        print("Blast Radius Warning: Topological breakages detected.")
        for change in breaking_changes: print(f" - {change}")
        sys.exit(1)
...
def scan_epistemic_quarantine(source: str) -> None:
    REGISTRY = ["SITD-Alpha: Non-Monotonic Epistemic Quarantine Isometry", "SITD-Beta: Defeasible Merkle-DAG Causal Bounding", "SITD-Gamma: Neurosymbolic Substrate Alignment", "Topologically Bounded Latent Spaces", "Pearlian Do-Operator"]
    def extract_descriptions(data: Any) -> list[str]:
        descriptions = []
        if isinstance(data, dict):
            for key, value in data.items():
                if key == "description" and isinstance(value, str): descriptions.append(value)
                descriptions.extend(extract_descriptions(value))
        elif isinstance(data, list):
            for item in data: descriptions.extend(extract_descriptions(item))
        return descriptions
    ...
    try:
        if source.startswith(("http://", "https://")):
            ...
            with open(source, encoding="utf-8") as f: schema_dict = json.load(f)
    except Exception as e:
...
def execute_ontological_transmutation() -> None:
    RENAMES = {
        "LiquidTypeContract": "AlgebraicRefinementContract", "EpistemicRewardModelPolicy": "EpistemicRewardGradientPolicy",
        "SemanticRelationalRecordState": "SemanticRelationalVectorState", "ManifoldAlignmentMetric": "ManifoldAlignmentMetricProfile",
        "IdeationPhase": "IdeationPhaseProfile", "StochasticStateNode": "StochasticNodeState", "HypothesisSuperposition": "HypothesisSuperpositionState",
        "StochasticTopology": "StochasticTopologyManifest", "TargetTopologyEnum": "TargetTopologyProfile",
        "CryptographicProvenanceMixin": "CryptographicProvenancePolicy", "ActiveInferenceEpoch": "ActiveInferenceEpochState",
        "ComputationalThermodynamics": "ComputationalThermodynamicsProfile", "SemanticMappingHeuristicProposal": "SemanticMappingHeuristicIntent",
        "CognitiveSwarmDeploymentMacro": "CognitiveSwarmDeploymentManifest", "PostCoordinatedSemanticConcept": "PostCoordinatedSemanticState",
        "EmpiricalStatisticalQualifier": "EmpiricalStatisticalProfile", "DempsterShaferBeliefVector": "DempsterShaferBeliefState",
        "SemanticRelationalRecord": "SemanticRelationalVectorState", "test_semantic_relational_record": "test_semantic_relational_vector",
    }
    for f in list(REPO_ROOT.rglob("*.py")):
        if "universal_ontology" in str(f) or ".venv" in str(f): continue
        content = f.read_text(encoding="utf-8")
        original = content
        for old, new in RENAMES.items(): content = content.replace(old, new)
        if content != original:
...
def inject_ast_semantic_anchors() -> None:
    ...
    with open(file_path, "w", encoding="utf-8") as f: f.write("".join(lines))
...
def main() -> None:
    ...
    if args.command == "evaluate_epistemic_compliance": evaluate_epistemic_compliance()
    elif args.command == "evaluate_architectural_manifold": evaluate_architectural_manifold()
    elif args.command == "evaluate_ast_instantiation_bounds": evaluate_ast_instantiation_bounds()
    elif args.command == "evaluate_topological_reachability": evaluate_topological_reachability()
    elif args.command == "calculate_semantic_differential": calculate_semantic_differential()
    elif args.command == "scan_epistemic_quarantine": scan_epistemic_quarantine(args.source)
    elif args.command == "inject_cryptographic_provenance": sys.exit(inject_cryptographic_provenance(args.files))
    elif args.command == "project_ontology_manifold": project_ontology_manifold()
    elif args.command == "execute_ontological_transmutation": execute_ontological_transmutation()
    elif args.command == "inject_ast_semantic_anchors": inject_ast_semantic_anchors()
