...
class TrendAnalysis(BaseModel):
    ...
    risk_score: int = Field(..., ge=0, max=100, description="Risk score from 0 to 100.")
...
def build_financial_analyst() -> ManifestV2:
    ...
    analyzer_builder.with_system_prompt(
        ...
        "Policy: No investment advice. "
        "Analyze the data neutrally."
    )
    ...
    manifest = ManifestV2(
        ...
        interface=InterfaceDefinition(
            inputs=fetcher_def.interface.inputs,
            outputs=writer_def.interface.outputs
        ),
        definitions={
            "FetcherAgent": fetcher_def,
            "AnalyzerAgent": analyzer_def,
            "WriterAgent": writer_def
        },
        workflow=workflow
    )
...
def simulate_execution(agent: ManifestV2, inputs: dict):
    """
    ...
    while current_step_id:
        ...
        if isinstance(step, AgentStep):
            ...
            print(f"   ℹ️ Non-Agent Step Type: {step.type}")
