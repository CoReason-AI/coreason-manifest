...
def _detect_utility_islands(flow: GraphFlow) -> list[ComplianceReport]:
    ...
    Malicious States Prevented:
        - Precludes latent payload activation and malicious topological bypasses generated by unreachable but active nodes.
...
def _is_guarded(target_node: AnyNode, flow: LinearFlow | GraphFlow) -> bool:
    ...
    # Detect implicit fallback routes to prevent security "topological bypasses"
    for node in nodes:
