...
class StatePatch(CoreasonBaseModel):
    op: PatchOperation = Field(description="The strict RFC 6902 JSON Patch operation, acting as a deterministic state vector mutation.")
    path: str = Field(description="The JSON pointer indicating the exact state vector to mutate deterministically.")
    value: Any | None = Field(default=None, description="The payload to insert or test, if applicable, for this deterministic state vector mutation.")


class StateDiff(CoreasonBaseModel):
    diff_id: str = Field(description="A Content Identifier (CID) acting as a cryptographic Lineage Watermark for this state differential.")
    author_node_id: str = Field(description="The exact Lineage Watermark of the agent or system that authored this state mutation.")
    ...
    vector_clock: dict[str, int] = Field(description="Causal history mapping of all known Lineage Watermarks to their latest logical mutation count at the time of authoring.")
...
class DefeasibleCascade(CoreasonBaseModel):
    cascade_id: str = Field(min_length=1, description="A Content Identifier (CID) acting as a cryptographic Lineage Watermark for this automated truth maintenance operation.")
    root_falsified_event_id: str = Field(description="The source BeliefUpdateEvent or HypothesisGenerationEvent Content Identifier (CID) that collapsed and triggered this cascade.")
    ...
    quarantined_event_ids: list[str] = Field(
        min_length=1, description="The strict list of downstream event Content Identifiers (CIDs) isolated and muted by this cascade to prevent Epistemic Contagion."
    )
...
class MigrationContract(CoreasonBaseModel):
    contract_id: str = Field(description="A Content Identifier (CID) acting as a cryptographic Lineage Watermark for this structural migration mapping.")
...
class RollbackRequest(CoreasonBaseModel):
    request_id: str = Field(description="A Content Identifier (CID) acting as a cryptographic Lineage Watermark for the causal rollback operation.")
    target_event_id: str = Field(description="The Content Identifier (CID) of the corrupted event in the EpistemicLedger to revert to.")
...
class TemporalCheckpoint(CoreasonBaseModel):
    checkpoint_id: str = Field(description="A Content Identifier (CID) acting as a cryptographic Lineage Watermark for the temporal anchor.")
