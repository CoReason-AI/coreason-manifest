...
def _validate_ssrf_safety(url: Any) -> Any:
    ...
    try:
        ...
        try:
            ...
            try:
                # Fallback to handle non-standard IPv4 representations (e.g. hex, octal, decimal bypasses)
                # socket.inet_aton parses a wide variety of these standard forms.
                ip_str = socket.inet_ntoa(socket.inet_aton(hostname))
