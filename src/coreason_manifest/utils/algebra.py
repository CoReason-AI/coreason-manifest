...
def _validate_ssrf_safety(url: Any) -> Any:
    ...
    try:
        ...
        try:
            # Normalize IPv4 addresses (handles hex, octal, int, and truncated formats)
            packed = socket.inet_aton(hostname)
            normalized_ip = socket.inet_ntoa(packed)
            ip = ipaddress.ip_address(normalized_ip)
        except OSError:
            try:
                # Fallback for IPv6
                packed = socket.inet_pton(socket.AF_INET6, hostname)
                normalized_ip = socket.inet_ntop(socket.AF_INET6, packed)
                ip = ipaddress.ip_address(normalized_ip)
            except OSError:
                try:
                    # Final fallback
