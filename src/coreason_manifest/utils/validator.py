    ...
    for node in nodes:
        ...
        if isinstance(node.resilience, str):
            ...
            if not ref.startswith("ref:"):
                errors.append(f"Resilience Error: Node '{node.id}' has invalid resilience reference '{ref}'. Must start with 'ref:'.")
            else:
                tmpl_id = ref[4:]
                if tmpl_id not in templates:
                    errors.append(f"Resilience Error: Node '{node.id}' references undefined supervision template ID '{tmpl_id}'.")

        # Check profile references (AgentNode)
        if isinstance(node, AgentNode) and isinstance(node.profile, str):
            if node.profile not in profile_ids:
        ...
        if isinstance(node, SwarmNode) and node.worker_profile not in profile_ids:
            errors.append(f"Integrity Error: SwarmNode '{node.id}' references undefined worker profile ID '{node.worker_profile}'.")
