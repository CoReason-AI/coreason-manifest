...
class CircuitBreakerConfig(CoReasonBaseModel):
    ...
    failure_rate_threshold: float = Field(
        0.5, ge=0.0, le=1.0, description="Trigger if error rate exceeds this (e.g. 0.5 = 50%)."
    )
...
class DriftConfig(CoReasonBaseModel):
    ...
    input_drift_threshold: float | None = Field(
        None, description="Max allowed distance from baseline for inputs. Lower is stricter."
    )
    output_drift_threshold: float | None = Field(None, description="Max allowed distance from baseline for outputs.")
    baseline_dataset_id: str | None = Field(None, description="Reference to the dataset used as the 'normal' baseline.")
...
class GuardrailsConfig(CoReasonBaseModel):
    ...
    spot_check_rate: float = Field(
        0.0, ge=0.0, le=1.0, description="Percentage of traces to flag for human review (0.0 to 1.0)."
    )
