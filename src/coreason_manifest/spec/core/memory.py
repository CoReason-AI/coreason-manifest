...
class EpisodicMemoryConfig(CoreasonModel):
    ...
    consolidation_interval_turns: int | None = Field(
        None,
        description="Number of conversation turns before the background loop compresses the journal. None means no auto-consolidation."
    )
...
class SemanticMemoryConfig(CoreasonModel):
    ...
    bitemporal_tracking: bool = Field(
        ...,
        description="If true, requires the runtime to track both 'valid_from' (business time) and 'recorded_at' (system time) metadata."
    )
...
class MemorySubsystem(CoreasonModel):
    ...
    semantic: SemanticMemoryConfig | None = Field(None, description="Configuration for Semantic Memory (Knowledge Graph).")
