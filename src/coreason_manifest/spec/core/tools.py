...
class Dependency(CoreasonModel):
    ...
    version: str | None = Field(
        None, description="Version constraint string.", examples=["^2.0.0", ">=1.0"]
    )
    manager: Literal["pip", "npm", "apt", "mcp"] = Field(
        ..., description="Package manager to use.", examples=["pip"]
    )
...
class ToolCapability(CoreasonModel):
    ...
    requires_approval: bool = Field(
        False, description="If True, human approval is required before execution."
    )
...
class ToolPack(CoreasonModel):
    ...
    tools: list[AnyTool] = Field(
        ..., description="List of tool capabilities provided by this pack.", examples=[[{"name": "calc", "type": "capability"}]]
    )
    dependencies: list[Dependency] = Field(
        default_factory=list, description="External package dependencies.", examples=[[{"name": "numpy", "manager": "pip"}]]
    )
