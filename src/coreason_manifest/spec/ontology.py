...
class C2PAAssertion(CoreasonBaseState):
    """A strictly typed representation of a C2PA standard assertion."""

    assertion_type: str = Field(description="The standard URI or type identifier for the assertion.")
    data: dict[str, Any] = Field(description="The strictly bounded payload dictionary for the assertion data.")
...
class C2PAIngredient(CoreasonBaseState):
    """A strictly typed representation of a C2PA standard ingredient."""

    ingredient_identifier: str = Field(description="The uniform resource identifier for the ingredient.")
    relationship: str = Field(description="The relationship between the ingredient and the claim.")
    metadata: dict[str, Any] | None = Field(default=None, description="Optional strictly bounded metadata.")

    @field_validator("metadata", mode="before")
    @classmethod
    def enforce_payload_topology(cls, v: Any) -> Any:
        return _validate_payload_bounds(v) if v is not None else v
...
class C2PAExportClaim(CoreasonBaseState):
    ...
    c2pa_assertions: list[C2PAAssertion] = Field(
        default_factory=list,
    ...
    c2pa_ingredients: list[C2PAIngredient] = Field(
        default_factory=list,
...
class SummarizeEpistemicReceiptRequest(BoundedJSONRPCIntent):
    """
    Standardized Model Context Protocol (MCP) tool schema that allows external LLMs
    to summarize epistemic receipts.
    """

    method: Literal["mcp.graph.summarize_epistemic_receipt"] = Field(
        default="mcp.graph.summarize_epistemic_receipt", description="Method to summarize epistemic receipts."
    )
...
class ExecutionNodeReceipt(CoreasonBaseState):
    ...
    environment_context: EnvironmentContextManifest | None = Field(
        default=None, description="The dynamic environment annotation at the moment of execution."
    )
    ...
    def generate_node_hash(self) -> str:
        ...
        payload = {
            ...
            "environment_context": self.environment_context.model_dump() if self.environment_context else None,
        }
