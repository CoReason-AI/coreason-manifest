...
class EpistemicProxyState[T](CoreasonBaseState):
    ...
    EPISTEMIC BOUNDS: The `proxy_id` is strictly bounded to a deterministic UUID string. The raw payload `T` is mathematically severed from serialization during prompt injection.

    MCP ROUTING TRIGGERS: Zero-Trust Isolation, Symbolic Firewall, Epistemic Quarantine, OpenSymbolicAI Projection
    """

    proxy_id: Annotated[str, StringConstraints(min_length=1, max_length=128, pattern="^[a-zA-Z0-9_.:-]+$")] = Field(
...
class EpistemicLedgerState(CoreasonBaseState):
    ...
    def _enforce_canonical_sort(self) -> Self:
        ...
        event_times = {event.event_cid: event.timestamp for event in self.history if hasattr(event, "event_cid") and hasattr(event, "timestamp")}
