...
class AcceleratorClass(StrEnum):
...
class StateVectorProfile(CoreasonBaseState):
    ...
    immutable_context: dict[Annotated[str, StringConstraints(max_length=255)], JsonPrimitiveState] = Field(
    ...
    @field_validator("mutable_manifold", "immutable_context", mode="before")
    @classmethod
...
class ComputeEngineProfile(CoreasonBaseState):
    ...
    tensor_manifold: Annotated[str, StringConstraints(max_length=2000)] = Field(
...
class ManifoldFidelityReceipt(CoreasonBaseState):
    r"""
...
class LatentScratchpadReceipt(CoreasonBaseState):
    ...
    def verify_referential_integrity(self) -> Self:
        ...
        for discarded_cid in self.discarded_branches:
            if discarded_cid not in explored_branch_ids:
...
class EphemeralNamespacePartitionState(CoreasonBaseState):
    ...
    partition_cid: Annotated[str, StringConstraints(min_length=1, max_length=128, pattern="^[a-zA-Z0-9_.:-]+$")] = Field(
...
class BypassReceipt(CoreasonBaseState):
    ...
    artifact_event_cid: Annotated[str, StringConstraints(min_length=1, max_length=128, pattern="^[a-zA-Z0-9_.:-]+$")] = (
        Field(
...
class CustodyReceipt(CoreasonBaseState):
    ...
    applied_policy_cid: Annotated[str, StringConstraints(min_length=1, max_length=255, pattern="^[a-zA-Z0-9_.:-]+$")] = (
        Field(
...
class DimensionalProjectionContract(CoreasonBaseState):
    ...
    source_tensor_manifold and target_tensor_manifold (both max_length=2000) identify the
    ...
    source_tensor_manifold: Annotated[str, StringConstraints(max_length=2000)] = Field(
        description="The native embedding model of the origin agent."
    )
    target_tensor_manifold: Annotated[str, StringConstraints(max_length=2000)] = Field(
...
class EpistemicArgumentClaimState(CoreasonBaseState):
    ...
    proponent_cid: Annotated[str, StringConstraints(min_length=1, max_length=128, pattern="^[a-zA-Z0-9_.:-]+$")] = Field(
...
class GenerativeTaxonomyManifest(CoreasonBaseState):
    ...
    root_node_cid: Annotated[str, StringConstraints(min_length=1, max_length=128, pattern="^[a-zA-Z0-9_.:-]+$")] = Field(
...
type AnyPresentationIntent = Annotated[
    InformationalIntent | DraftingIntent | AdjudicationIntent | EscalationIntent, Field(discriminator="topology_class")
]
...
class HardwareProfile(CoreasonBaseState):
    ...
    EPISTEMIC BOUNDS: VRAM allocation is physically bounded by min_vram_gb (gt=0.0). The literal enumerations ComputeTier and AcceleratorClass mathematically prevent the hallucination of non-existent silicon. The provider_whitelist is deterministically sorted for invariant RFC 8785 hashing.
    ...
    accelerator_type: AcceleratorClass = Field(
        default=AcceleratorClass.BF16_TENSOR,
...
class MCPServerManifest(CoreasonBaseState):
    ...
    def enforce_stdio_supply_chain_lock(self) -> Self:
        ...
        """
        if getattr(self.transport, "topology_class", None) == "stdio" and (not self.binary_hash):
            raise ValueError(
...
type AnyActionSpaceCapability = Annotated[
    ToolManifest | MCPServerManifest | EphemeralNamespacePartitionState, Field(discriminator="topology_class")
]
...
class EpistemicProvenanceReceipt(CoreasonBaseState):
    ...
    fidelity_receipt_hash: Annotated[str, StringConstraints(max_length=64)] | None = Field(
        default=None, description="Cryptographic pointer back to the ManifoldFidelityReceipt generated at the Input Gate."
    )
...
class ProcessRewardContract(CoreasonBaseState):
    ...
    evaluator_tensor_manifold: Annotated[str, StringConstraints(max_length=2000)] | None = Field(
        default=None,
...
class VectorEmbeddingState(CoreasonBaseState):
    ...
    tensor_manifold (max_length=2000) traces embedding provenance. The dimensionality
...
class EpistemicLedgerState(CoreasonBaseState):
    ...
    def _enforce_canonical_sort(self) -> Self:
        ...
        object.__setattr__(self, "active_cascades", sorted(self.active_cascades, key=operator.attrgetter("cascade_cid")))
        return self
...
class NeurosymbolicInferenceRequest(CoreasonBaseState):
    ...
    fidelity_receipt: ManifoldFidelityReceipt = Field(
...
ManifoldFidelityReceipt.model_rebuild()
