...
class SideEffectProfile(CoreasonBaseState):
    ...
    CAUSAL AFFORDANCE: Instructs the orchestrator's graph traversal engine (e.g., MCTS) whether a tool can be safely re-evaluated concurrently (`is_cidempotent`) or if it induces irreversible kinetic entropy (`mutates_state`).
...
class CustodyReceipt(CoreasonBaseState):
    ...
    payload_cid: Annotated[str, StringConstraints(min_length=1, max_length=255, pattern="^[a-zA-Z0-9_.:-]+$")] = Field(
