...
class BoundedInterventionScopePolicy(CoreasonBaseState):
    ...
    json_schema_whitelist: dict[
        ...
        JsonPrimitiveState,
    ] = Field(description="Strict JSON Schema constraints for the human's input. AGENT INSTRUCTION: Payload volume is strictly limited to an absolute $O(N)$ limit of 10,000 nodes and a maximum recursion depth of 10 to prevent VRAM exhaustion.")
...
class BoundedJSONRPCIntent(CoreasonBaseState):
    ...
    params: dict[Annotated[str, StringConstraints(max_length=255)], JsonPrimitiveState] | None = Field(
        max_length=86400000, default=None, description="Payload parameters. AGENT INSTRUCTION: Payload volume is strictly limited to an absolute $O(N)$ limit of 10,000 nodes and a maximum recursion depth of 10 to prevent VRAM exhaustion."
    )
...
class BargeInInterruptEvent(BaseStateEvent):
    ...
    retained_partial_payload: dict[Annotated[str, StringConstraints(max_length=255)], JsonPrimitiveState] | str | None = Field(
        max_length=100000,
        default=None,
...
class ExecutionNodeReceipt(CoreasonBaseState):
    ...
    inputs: JsonPrimitiveState = Field(description="The inputs provided to the execution node. AGENT INSTRUCTION: Payload volume is strictly limited to an absolute $O(N)$ limit of 10,000 nodes and a maximum recursion depth of 10 to prevent VRAM exhaustion.")
    outputs: JsonPrimitiveState = Field(description="The outputs generated by the execution node. AGENT INSTRUCTION: Payload volume is strictly limited to an absolute $O(N)$ limit of 10,000 nodes and a maximum recursion depth of 10 to prevent VRAM exhaustion.")
...
class MCPPromptReferenceState(CoreasonBaseState):
    ...
    arguments: dict[Annotated[str, StringConstraints(max_length=255)], JsonPrimitiveState] = Field(
        max_length=1000000000, default_factory=dict, description="Arguments to fill the prompt template. AGENT INSTRUCTION: Payload volume is strictly limited to an absolute $O(N)$ limit of 10,000 nodes and a maximum recursion depth of 10 to prevent VRAM exhaustion."
    )
...
class ToolInvocationEvent(BaseStateEvent):
    ...
    parameters: dict[Annotated[str, StringConstraints(max_length=255)], JsonPrimitiveState] = Field(
        max_length=1000000000, description="The intended JSON-RPC payload. AGENT INSTRUCTION: Payload volume is strictly limited to an absolute $O(N)$ limit of 10,000 nodes and a maximum recursion depth of 10 to prevent VRAM exhaustion."
    )
