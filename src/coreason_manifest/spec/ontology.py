...
def _canonicalize_payload(obj: Any) -> Any:
    ...
    typ = type(obj)
    if typ is dict:
        return {k: _canonicalize_payload(v) if type(v) in (dict, list) else v for k, v in obj.items() if v is not None}
    if typ is list:
        return [_canonicalize_payload(v) if type(v) in (dict, list) else v for v in obj]
    return obj
