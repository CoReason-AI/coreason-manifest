...
class SpatialKinematicActionIntent(CoreasonBaseState):
    ...
    noise_override: KinematicNoiseProfile | None = Field(
        default=None, description="The specific Brownian motion envelope to apply to this kinematic trajectory."
    )
...
class KinematicNoiseProfile(CoreasonBaseState):
    """
    AGENT INSTRUCTION: Implements Brownian motion and Fitts's Law variances to simulate human-like imprecision in spatial execution. As a ...Profile suffix, this is a declarative property descriptor.

    CAUSAL AFFORDANCE: Instructs the kinematic actuation engine to dynamically perturb trajectory control points, injecting continuous stochastic noise to bypass bot-evasive heuristics.

    EPISTEMIC BOUNDS: Bounded by strict mathematical float limits. `pink_noise_amplitude` must be in [0.0, 1.0]. `fitts_law_variance_ms` is a temporal delta bounded to le=10000.

    MCP ROUTING TRIGGERS: Brownian Motion, Fitts's Law, Stochastic Perturbation, Kinematic Entropy, Exogenous Actuation
    """

    pink_noise_amplitude: float = Field(
        ge=0.0, le=1.0, description="The strictly typed scalar governing the amplitude of spatial perturbation."
    )
    fitts_law_variance_ms: int = Field(
        le=10000, ge=0, description="The mathematical variance applied to the temporal duration of a trajectory."
    )


class EnvironmentalSpoofingProfile(CoreasonBaseState):
    """
    AGENT INSTRUCTION: Defines a deterministic topological envelope for hardware and OS-level emulation. As a ...Profile suffix, this is a declarative property descriptor.

    CAUSAL AFFORDANCE: Authorizes the orchestrator to dynamically inject spoofed environmental telemetry (e.g., user agents, canvas hashes) into a simulated exogenous plane (e.g., headless browser) to evade epistemic fingerprinting.

    EPISTEMIC BOUNDS: Hardware geometries are bounded: `screen_resolution` must map to physical integer coordinates (max 100000). Fingerprints are bounded to 128-char SHA-256 or explicit CID geometries.

    MCP ROUTING TRIGGERS: Topological Spoofing, Canvas Fingerprinting, OS-Level Emulation, Hardware Virtualization, Epistemic Evastion
    """

    screen_resolution: tuple[int, int] = Field(description="The spoofed Euclidean bounds of the display.")
    user_agent_override: str = Field(
        max_length=2000, description="The explicit browser/OS telemetry injected into the header."
    )
    webgl_entropy_seed_hash: str | None = Field(
        default=None,
        min_length=1,
        max_length=128,
        pattern="^[a-f0-9]{64}$",
        description="The optional SHA-256 seed used to deterministically perturb WebGL readouts.",
    )


class AdversarialEmulationProfile(CoreasonBaseState):
    """
    AGENT INSTRUCTION: Aggregates stochastic and environmental profiles to establish a unified Threat Emulation geometry. As a ...Profile suffix, this is a declarative property descriptor.

    CAUSAL AFFORDANCE: Instructs the simulation matrix to fully wrap an execution manifold in a hostile or obfuscated physical context, linking noise injection (`KinematicNoiseProfile`) with hardware spoofing (`EnvironmentalSpoofingProfile`).

    EPISTEMIC BOUNDS: Relies entirely on the strict validation cascades of its nested structural members to prevent probability wave overflow or payload bombing.

    MCP ROUTING TRIGGERS: Threat Emulation, Adversarial Virtualization, Execution Manifold Wrapping, Epistemic Obfuscation
    """

    kinematic_noise: KinematicNoiseProfile | None = Field(
        default=None, description="The continuous spatial perturbation profile."
    )
    environmental_spoof: EnvironmentalSpoofingProfile | None = Field(
        default=None, description="The OS/Hardware spoofing envelope."
    )
    rotation_ttl_seconds: int = Field(
        le=86400,
        ge=0,
        description="The exact temporal threshold before the emulation environment must be structurally rotated.",
    )
...
class AgentNodeProfile(BaseNodeProfile):
    ...
    adversarial_emulation: AdversarialEmulationProfile | None = Field(
        default=None,
        description="The declarative structural envelope authorizing physical hardware spoofing and kinematic noise injection for this agent.",
    )
