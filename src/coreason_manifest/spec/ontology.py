...
class EnvironmentContextManifest(CoreasonBaseState):
    os_version: str = Field(description="The exact operating system version.")
    python_version: str = Field(description="The exact Python version string.")
    active_dependencies: dict[str, str] = Field(description="Dictionary of active dependencies mapped to exact hashes.")
    gpu_vram: int | None = Field(default=None, description="The GPU VRAM available, if any.")
    random_seed_state: int | None = Field(default=None, description="The random seed state to ensure reproducibility.")
...
class LatentScratchpadReceipt(CoreasonBaseState):
    ...
    environment_context: EnvironmentContextManifest | None = Field(
        default=None, description="The strictly deterministic environment state at time of scratchpad execution."
    )
...
class ComplianceRatingScheme(CoreasonBaseState):
    c2pa_metadata_present: bool = Field(description="Indicates if C2PA metadata is bound to the payload.")
    licensing_constraints: list[str] = Field(
        description="Strictly bound mathematical representations of the licensing constraints."
    )
    opt_out_mechanisms: list[str] = Field(
        description="List of verified opt-out mechanisms adhered to by the data."
    )
    final_grade: Literal["A", "B", "C", "D", "E", "F", "G"] = Field(
        description="The deterministic final grade evaluated for the dataset."
    )

    @model_validator(mode="after")
    def _enforce_crs_circuit_breaker(self) -> "ComplianceRatingScheme":
        if self.final_grade in ["D", "E", "F", "G"]:
            raise ValueError(
                f"CRITICAL COMPLIANCE VIOLATION: The CRS grade {self.final_grade} falls below "
                "the 'C' threshold. Ingestion is strictly forbidden."
            )
        object.__setattr__(self, "licensing_constraints", sorted(self.licensing_constraints))
        object.__setattr__(self, "opt_out_mechanisms", sorted(self.opt_out_mechanisms))
        return self

class ExternalPayloadManifest(CoreasonBaseState):
    payload_id: str = Field(description="The deterministic identifier for this external data ingress.")
    compliance_rating: ComplianceRatingScheme = Field(
        description="The mathematically enforced ethical firewall for the incoming data."
    )
    raw_content_hash: str = Field(description="The canonical SHA-256 hash of the pure ingested data.")
...
class ExecutionNodeReceipt(CoreasonBaseState):
    ...
    environment_context: EnvironmentContextManifest | None = Field(
        default=None, description="The exact hardware and software state at the time of execution."
    )
...
EnvironmentContextManifest.model_rebuild()
ComplianceRatingScheme.model_rebuild()
ExternalPayloadManifest.model_rebuild()
