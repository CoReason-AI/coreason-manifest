...
valid_node_cid_st = st.from_regex(r"^did:[a-z0-9]+:[a-zA-Z0-9.\-_:]+$", fullmatch=True)
node_st = st.builds(CognitiveSystemNodeProfile, description=st.text())


@st.composite
def nodes_dict_st(draw: st.DrawFn) -> dict[str, Any]:
    return draw(st.dictionaries(keys=valid_node_cid_st, values=node_st, min_size=1, max_size=10))
...
def test_causal_directed_edge_state_fuzz(source: str, target: str, topology_class: Any) -> None:
    try:
        CausalDirectedEdgeState(source_variable=source, target_variable=target, edge_class=topology_class)
        if source == target:
