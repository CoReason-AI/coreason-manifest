...
def test_epistemic_attention_state_validate_unit_vector(dx: float, dy: float, dz: float) -> None:
    """
    Fuzz EpistemicAttentionState unit vector validation.
    """
    import math
    magnitude = math.hypot(dx, dy, dz)
    
    if magnitude == 0.0:
        with pytest.raises(ValueError, match="Kinematic Violation: Attention Ray direction cannot be a zero vector"):
            EpistemicAttentionState(
                origin=SE3TransformProfile(reference_frame_cid="frame-123", x=0.0, y=0.0, z=0.0),
        ...
        assert math.isclose(math.hypot(*state.direction_unit_vector), 1.0, abs_tol=1e-3)
...
def test_epistemic_attention_state_canonical_sort(nodes: list[str]) -> None:
    """
    Test canonical sorting of intersected_node_cids.
    """
    # Create with unsorted list
    unsorted_nodes = list(nodes)
    # Ensure it's not sorted
    if unsorted_nodes == sorted(unsorted_nodes):
        unsorted_nodes.reverse()
        
    state = EpistemicAttentionState(
        origin=SE3TransformProfile(reference_frame_cid="frame-123", x=0.0, y=0.0, z=0.0),
        direction_unit_vector=(1.0, 0.0, 0.0),
        intersected_node_cids=unsorted_nodes,
    )
    assert state.intersected_node_cids == sorted(unsorted_nodes)
