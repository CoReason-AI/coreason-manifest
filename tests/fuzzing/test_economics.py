...
@given(
    probs=st.lists(st.floats(allow_nan=False, allow_infinity=False), min_size=1, max_size=10)
)
...
@given(
    max_budget=st.integers(min_value=-2000000000, max_value=2000000000)
)
...
def test_token_burn_receipt(burn, inp, out):
    tbr = TokenBurnReceipt(
        event_id="e1", timestamp=100.0, tool_invocation_id="t1",
        input_tokens=inp, output_tokens=out, burn_magnitude=burn
    )
...
def test_routing_frontier_policy(lat, cost, carbon):
    rfp = RoutingFrontierPolicy(
        max_latency_ms=lat, max_cost_magnitude_per_token=cost, min_capability_score=0.5,
        tradeoff_preference="balanced", max_carbon_intensity_gco2eq_kwh=carbon
    )
...
@given(
    escrow=st.integers(min_value=-2000000000, max_value=2000000000)
)
def test_escrow_policy(escrow):
    ep = EscrowPolicy(
        escrow_locked_magnitude=escrow,
        release_condition_metric="rc1",
        refund_target_node_id="n1"
    )
