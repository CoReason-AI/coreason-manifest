...
def test_document_layout_region_state(
    block_cid: str, block_class: Any, token_span_start: int, token_span_end: int
) -> None:
...
def test_viewport_projection_perspective_fov() -> None:
    ...
    with pytest.raises(ValidationError) as exc_info:
        ViewportProjectionContract(
            projection_class="perspective", clipping_plane_near=0.1, clipping_plane_far=10.0, field_of_view_degrees=None
        )
