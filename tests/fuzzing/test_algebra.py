...
def test_calculate_latent_alignment_fuzz(v1_floats: list[float], v2_floats: list[float]) -> None:
    ...
    v1 = VectorEmbeddingState(
        vector_base64=base64.b64encode(v1_packed).decode(), dimensionality=dim, neural_matrix_name="fuzz"
    )
    v2 = VectorEmbeddingState(
        vector_base64=base64.b64encode(v2_packed).decode(), dimensionality=dim, neural_matrix_name="fuzz"
    )
