...
from coreason_manifest.utils.integrity import compute_state_hash, create_merkle_node, verify_merkle_proof
from coreason_manifest.utils.loader import CitadelLoader
...
def test_gatekeeper_missing_profile_definition() -> None:
    ...
    node = AgentNode(id="agent1", type="agent", metadata={}, supervision=None, profile="missing_profile_id", tools=[])
...
def test_gatekeeper_resolved_profile_definition() -> None:
    ...
    # Node references ID
    node = AgentNode(id="agent1", type="agent", metadata={}, supervision=None, profile="profile_1", tools=[])
    graph = Graph(nodes={"agent1": node}, edges=[])
...
def test_integrity_multiple_roots() -> None:
    ...
    # If verify_merkle_proof supports lists of disconnected nodes
    # (which a DAG list might contain if topographically sorted)
    chain = [root1, root2]
