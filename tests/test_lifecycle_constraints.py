...
def create_flow(status="draft", entry_point=None, nodes=None, edges=None):
    if nodes is None:
        nodes = {}
    if edges is None:
        edges = []
...
def test_draft_mode_disconnected_safe_nodes():
...
def test_draft_mode_disconnected_dangerous_node():
    ...
    adds_node = [op for op in patch if op["op"] == "add" and "nodes" in op["path"] and "guard_node2" in op["path"]]
    adds_edge = [op for op in patch if op["op"] == "add" and "edges" in op["path"]]

    assert len(adds_node) > 0
    assert len(adds_edge) > 0

def test_published_mode_missing_entry_point():
