...
def test_graph_event_polymorphism():
    data = [
        {
...
def test_graph_event_serialization():
...
def test_migration_error_event():
...
def test_migration_all_types():
    ...
    # NODE_START
    e1 = GraphEventNodeStart(
        run_id="r", trace_id="t", node_id="n", timestamp=1.0, payload={"p": 1}
    )
    ...
    # NODE_DONE
    e2 = GraphEventNodeDone(
        run_id="r", trace_id="t", node_id="n", timestamp=1.0, output={"o": 2}
    )
    ...
    # COUNCIL_VOTE
    e4 = GraphEventCouncilVote(
        run_id="r", trace_id="t", node_id="n", timestamp=1.0, votes={"alice": True}
    )
    ...
    # NODE_RESTORED
    e5 = GraphEventNodeRestored(
        run_id="r", trace_id="t", node_id="n", timestamp=1.0, status="active"
    )
