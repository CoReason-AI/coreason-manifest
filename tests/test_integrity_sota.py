...
def test_integrity_chain_valid() -> None:
    # Genesis
    genesis = create_merkle_node(previous_hashes=["0"], node_id="genesis", blackboard={"step": 0})
    chain = [genesis]

    # Step 1
    node1 = create_merkle_node(previous_hashes=[genesis.compute_hash()], node_id="node1", blackboard={"step": 1})
    chain.append(node1)

    # Step 2
    node2 = create_merkle_node(previous_hashes=[node1.compute_hash()], node_id="node2", blackboard={"step": 2})
    chain.append(node2)
...
def test_integrity_chain_tampered() -> None:
    ...
    fake_node1 = create_merkle_node(
        previous_hashes=[genesis.compute_hash()],
        node_id="node1",
