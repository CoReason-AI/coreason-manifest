...
def test_browser_dom_state_topological_violations(url: str) -> None:
    with pytest.raises(ValidationError, match=r"(SSRF restricted IP detected|SSRF topological violation detected|Security Validation Failed: Unresolvable or invalid host)"):
        BrowserDOMState(
...
def test_browser_dom_state_mathematical_bounds(url: str) -> None:
    with pytest.raises(ValidationError, match=r"(SSRF restricted IP detected|Security Validation Failed: Unresolvable or invalid host)"):
        BrowserDOMState(
            current_url=url, viewport_size=(1920, 1080), dom_hash="a" * 64, accessibility_tree_hash="b" * 64
        )
