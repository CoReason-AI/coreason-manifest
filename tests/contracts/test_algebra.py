...
@pytest.mark.parametrize(
    ...
    [
        ...
        ({"op": "copy", "path": "/nested/new_key", "from": "/arr/10"}, "Invalid from_path operation: Invalid index"),
        (
            {"op": "copy", "path": "/nested/new_key", "from": "/nested/key/invalid"},
            "Invalid from_path operation: Target is not dict or list",
        ),
    ],
)
