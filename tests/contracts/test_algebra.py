...
def test_apply_state_differential_array_dash_errors() -> None:
    """Test that trying to read/remove/replace from array end index '-' raises ValueError."""
    base_state = {"metrics": [10, 20, 30]}

    # Test 'replace' to '-'
    patch_replace_dash = StateMutationIntent(op="replace", path="/metrics/-", value=99)
    manifest = StateDifferentialManifest(
        diff_id="did:web:patch-err-1",
        author_node_id="did:web:node-1",
        lamport_timestamp=1,
        vector_clock={},
        patches=[patch_replace_dash]
    )
    with pytest.raises(ValueError, match="Cannot replace at path /metrics/-: Cannot extract from end of array"):
        apply_state_differential(base_state, manifest)

    # Test 'remove' to '-'
    patch_remove_dash = StateMutationIntent(op="remove", path="/metrics/-")
    manifest = StateDifferentialManifest(
        diff_id="did:web:patch-err-2",
        author_node_id="did:web:node-1",
        lamport_timestamp=1,
        vector_clock={},
        patches=[patch_remove_dash]
    )
    with pytest.raises(ValueError, match="Cannot remove from path /metrics/-: Cannot remove from end of array"):
        apply_state_differential(base_state, manifest)

    # Test 'copy' from '-' (invokes extract_from_target)
    patch_copy_dash = StateMutationIntent(op="copy", path="/metrics/0", value="/metrics/-")
    manifest = StateDifferentialManifest(
        diff_id="did:web:patch-err-3",
        author_node_id="did:web:node-1",
        lamport_timestamp=1,
        vector_clock={},
        patches=[patch_copy_dash]
    )
    with pytest.raises(ValueError, match="Invalid from_path operation: Cannot extract from end of array"):
        apply_state_differential(base_state, manifest)
