...
def test_bounded_jsonrpc_intent_none_params() -> None:
    from coreason_manifest.spec.ontology import BoundedJSONRPCIntent

    intent = BoundedJSONRPCIntent(jsonrpc="2.0", method="test", params=None)
    assert intent.params == {}


def test_bounded_jsonrpc_intent_invalid_params_type() -> None:
    from coreason_manifest.spec.ontology import BoundedJSONRPCIntent

    with pytest.raises(ValidationError, match=r"params must be a dictionary"):
        BoundedJSONRPCIntent(jsonrpc="2.0", method="test", params=[1, 2, 3])  # type: ignore


def test_bounded_jsonrpc_intent_depth_exceeded() -> None:
    from coreason_manifest.spec.ontology import BoundedJSONRPCIntent

    # Create dict with depth 11
    d = {}
    current = d
    for _ in range(11):
        current["k"] = {}
    ...
    with pytest.raises(ValidationError, match=r"JSON payload exceeds maximum depth of 10"):
        BoundedJSONRPCIntent(jsonrpc="2.0", method="test", params=d)


def test_bounded_jsonrpc_intent_dict_keys_exceeded() -> None:
    from coreason_manifest.spec.ontology import BoundedJSONRPCIntent

    # Create dict with 101 keys
    d = {f"k{i}": i for i in range(101)}

    with pytest.raises(ValidationError, match=r"Dictionary exceeds maximum of 100 keys"):
        BoundedJSONRPCIntent(jsonrpc="2.0", method="test", params=d)


def test_bounded_jsonrpc_intent_dict_key_length_exceeded() -> None:
    from coreason_manifest.spec.ontology import BoundedJSONRPCIntent

    # Create dict with key longer than 1000
    d = {"k" * 1001: 1}

    with pytest.raises(ValidationError, match=r"Dictionary key exceeds maximum length of 1000"):
        BoundedJSONRPCIntent(jsonrpc="2.0", method="test", params=d)


def test_bounded_jsonrpc_intent_list_size_exceeded() -> None:
    from coreason_manifest.spec.ontology import BoundedJSONRPCIntent

    # Create list with 1001 elements
    d = {"k": [1] * 1001}

    with pytest.raises(ValidationError, match=r"List exceeds maximum of 1000 elements"):
        BoundedJSONRPCIntent(jsonrpc="2.0", method="test", params=d)


def test_bounded_jsonrpc_intent_string_length_exceeded() -> None:
    from coreason_manifest.spec.ontology import BoundedJSONRPCIntent

    # Create string longer than 10000
    d = {"k": "s" * 10001}

    with pytest.raises(ValidationError, match=r"String exceeds maximum length of 10000 characters"):
        BoundedJSONRPCIntent(jsonrpc="2.0", method="test", params=d)
...
def test_browser_dom_state_file_protocol_blocked() -> None:
    from coreason_manifest.spec.ontology import BrowserDOMState

    with pytest.raises(ValidationError, match=r"SSRF topological violation detected: file:// schema is forbidden"):
        BrowserDOMState(
...
def test_browser_dom_state_local_hostname_blocked() -> None:
    from coreason_manifest.spec.ontology import BrowserDOMState

    with pytest.raises(ValidationError, match=r"SSRF topological violation detected: localhost"):
        BrowserDOMState(
...
def test_browser_dom_state_private_ip_blocked() -> None:
    from coreason_manifest.spec.ontology import BrowserDOMState

    with pytest.raises(ValidationError, match=r"SSRF mathematical bound violation detected: 10.0.0.1"):
        BrowserDOMState(
...
def test_browser_dom_state_loopback_ip_blocked() -> None:
    from coreason_manifest.spec.ontology import BrowserDOMState

    with pytest.raises(ValidationError, match=r"SSRF mathematical bound violation detected: 127.0.0.1"):
        BrowserDOMState(
...
def test_browser_dom_state_hex_ip_blocked() -> None:
    from coreason_manifest.spec.ontology import BrowserDOMState
...
def test_browser_dom_state_octal_ip_blocked() -> None:
    from coreason_manifest.spec.ontology import BrowserDOMState
...
def test_browser_dom_state_integer_ip_blocked() -> None:
    from coreason_manifest.spec.ontology import BrowserDOMState
...
def test_browser_dom_state_no_hostname() -> None:
    ...
    # no hostname like 'data:text/html,test'
    state = BrowserDOMState(
...
def test_browser_dom_state_invalid_ip_bypasses() -> None:
    from coreason_manifest.spec.ontology import BrowserDOMState

    # Overflow error parsing IP
    state = BrowserDOMState(
...
def test_knowledge_graph_mutation_intent_non_append_only_oom() -> None:
    from coreason_manifest.spec.ontology import ContinuousMutationPolicy
