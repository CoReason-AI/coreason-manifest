...
def test_federated_capability_attestation_receipt_restricted_locks() -> None:
    ...
    FederatedCapabilityAttestationReceipt(
        attestation_id="att1", target_topology_id="did:ex:1", authorized_session=session_with_keys, governing_sla=sla
    )
...
def test_epistemic_transmutation_task_validate_grounding_density_for_visuals() -> None:
    ...
    sla = EpistemicCompressionSLA(
        strict_probability_retention=True, max_allowed_entropy_loss=0.0, required_grounding_density="dense"
    )
    EpistemicTransmutationTask(
        task_id="t1", artifact_event_id="e1", target_modalities=["raster_image", "tabular_grid"], compression_sla=sla
    )

    # Invalid
    sla_invalid = EpistemicCompressionSLA(
        strict_probability_retention=True, max_allowed_entropy_loss=0.0, required_grounding_density="sparse"
    )
    with pytest.raises(
        ValidationError,
        ...
        EpistemicTransmutationTask(
            task_id="t2", artifact_event_id="e2", target_modalities=["raster_image"], compression_sla=sla_invalid
        )
...
def test_task_award_receipt_validate_escrow_bounds() -> None:
    from coreason_manifest.spec.ontology import EscrowPolicy, TaskAwardReceipt
...
def test_dynamic_routing_manifest_validate_modality_alignment() -> None:
    ...
    profile = GlobalSemanticProfile(
        artifact_event_id="e1", detected_modalities=["text", "tabular_grid"], token_density=10
    )
    ...
    with pytest.raises(
        ValidationError,
        match=r"Epistemic Violation: Cannot route to subgraph 'raster_image'.*",
    ):
        DynamicRoutingManifest(
            manifest_id="m1",
            artifact_profile=profile,
...
def test_dynamic_routing_manifest_validate_conservation_of_custody() -> None:
    from coreason_manifest.spec.ontology import BypassReceipt, DynamicRoutingManifest, GlobalSemanticProfile
    ...
    with pytest.raises(
        ValidationError,
        match=r"Merkle Violation: BypassReceipt artifact_event_id does not match the root artifact_profile.",
    ):
        DynamicRoutingManifest(
            manifest_id="m1",
            artifact_profile=profile,
...
def test_execution_span_receipt_sort_events() -> None:
    ...
    receipt = ExecutionSpanReceipt(
        trace_id="t1", span_id="s1", name="n1", start_time_unix_nano=0, events=[event1, event2]
    )
...
def test_structural_causal_graph_profile_sort_arrays() -> None:
    ...
    profile = StructuralCausalGraphProfile(
        observed_variables=["v2", "v1"], latent_variables=["l2", "l1"], causal_edges=[edge1, edge2]
    )
...
def test_neural_audit_attestation_receipt_sort_arrays() -> None:
    ...
    receipt = NeuralAuditAttestationReceipt(audit_id="a1", layer_activations={1: [act1, act2]})
...
def test_composite_node_profile_sort_arrays() -> None:
    ...
    profile = CompositeNodeProfile(
        description="d1", topology=topology, input_mappings=[in1, in2], output_mappings=[out1, out2]
    )
...
def test_prediction_market_state_sort_arrays() -> None:
    from coreason_manifest.spec.ontology import HypothesisStakeReceipt, PredictionMarketState

    stake1 = HypothesisStakeReceipt(
        agent_id="agent2", target_hypothesis_id="h1", staked_magnitude=100, implied_probability=0.5
    )
    stake2 = HypothesisStakeReceipt(
        agent_id="agent1", target_hypothesis_id="h1", staked_magnitude=200, implied_probability=0.8
    )
...
def test_compute_provisioning_intent_sort_arrays() -> None:
    ...
    intent = ComputeProvisioningIntent(max_budget=100.0, required_capabilities=["c", "a", "b"])
...
def test_semantic_firewall_policy_sort_arrays() -> None:
    ...
    policy = SemanticFirewallPolicy(max_input_tokens=100, forbidden_intents=["c", "a", "b"], action_on_violation="drop")
...
def test_information_flow_policy_sort_arrays() -> None:
    ...
    policy = InformationFlowPolicy(policy_id="p1", rules=[r1, r2], latent_firewalls=[f1, f2])
...
def test_mcp_server_binding_profile_sort_arrays() -> None:
    ...
    profile = MCPServerBindingProfile(
        server_id="s1", transport=transport, required_capabilities=["tools", "prompts", "resources"]
    )
...
def test_steady_state_hypothesis_state_sort_arrays() -> None:
    ...
    state = SteadyStateHypothesisState(
        expected_max_latency=10.0, max_loops_allowed=5, required_tool_usage=["c", "a", "b"]
    )
...
def test_chaos_experiment_task_sort_arrays() -> None:
    ...
    s1 = ExogenousEpistemicEvent(
        shock_id="s2", target_node_hash="a" * 64, bayesian_surprise_score=0.5, synthetic_payload={}, escrow=escrow
    )
    s2 = ExogenousEpistemicEvent(
        shock_id="s1", target_node_hash="a" * 64, bayesian_surprise_score=0.5, synthetic_payload={}, escrow=escrow
    )

    task = ChaosExperimentTask(experiment_id="e1", hypothesis=hypothesis, faults=[f1, f2], shocks=[s1, s2])
...
def test_hypothesis_generation_event_sort_arrays() -> None:
    from coreason_manifest.spec.ontology import FalsificationContract, HypothesisGenerationEvent
...
def test_system_1_reflex_policy_sort_arrays() -> None:
    ...
    policy = System1ReflexPolicy(confidence_threshold=0.8, allowed_passive_tools=["t2", "t1", "t3"])

    assert policy.allowed_passive_tools == ["t1", "t2", "t3"]


def test_auction_state_sort_bids() -> None:
    from coreason_manifest.spec.ontology import AgentBidIntent, AuctionState, TaskAnnouncementIntent

    announcement = TaskAnnouncementIntent(task_id="t1", max_budget_magnitude=1000)

    b1 = AgentBidIntent(
        agent_id="agent2",
        estimated_cost_magnitude=500,
        estimated_latency_ms=10,
        estimated_carbon_gco2eq=0.0,
        confidence_score=0.9,
    )
    b2 = AgentBidIntent(
        agent_id="agent1",
        estimated_cost_magnitude=400,
        estimated_latency_ms=15,
        estimated_carbon_gco2eq=0.0,
        confidence_score=0.8,
    )

    state = AuctionState(announcement=announcement, bids=[b1, b2], clearing_timeout=100, minimum_tick_size=1.0)
...
def test_trace_export_manifest_sort_spans() -> None:
    from coreason_manifest.spec.ontology import ExecutionSpanReceipt, TraceExportManifest
    ...
    manifest = TraceExportManifest(batch_id="b1", spans=[s1, s2])
...
def test_utility_justification_graph_receipt_interlocks() -> None:
    ...
    ensemble = EnsembleTopologyProfile(
        concurrent_branch_ids=["did:ex:id1", "did:ex:id2"], fusion_function="weighted_consensus"
    )
    ...
    with pytest.raises(ValidationError, match=r"Tensor Poisoning Detected: Vector 'v1' contains invalid float"):
        UtilityJustificationGraphReceipt(optimizing_vectors={"v1": float("inf")}, superposition_variance_threshold=0.5)
...
def test_system2_remediation_intent_sort_arrays() -> None:
    ...
    intent = System2RemediationIntent(
        fault_id="f1", target_node_id="did:ex:1", failing_pointers=["/c", "/a", "/b"], remediation_prompt="fix it"
    )
