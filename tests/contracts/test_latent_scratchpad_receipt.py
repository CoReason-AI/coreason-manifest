...
def valid_scratchpad_strategy(draw: st.DrawFn) -> dict[str, Any]:
    ...
    explored = [
        ThoughtBranchState(branch_cid=b_cid, latent_content_hash="a" * 64, prm_score=0.9) for b_cid in branch_cids
    ]

    discarded = draw(st.lists(st.sampled_from(branch_cids), max_size=len(branch_cids), unique=True))
    resolution_cid = draw(st.one_of(st.none(), st.sampled_from(branch_cids)))
