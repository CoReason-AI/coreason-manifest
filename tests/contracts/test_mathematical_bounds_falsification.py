...
def test_thermodynamic_bounds_reject_nan():
...
def test_spatial_bounds_reject_inf():
...
def test_spatial_bounds_reject_nan():
...
def test_confidence_interval_contradiction():
...
def test_superposition_variance_zero():
    with pytest.raises(ValidationError, match="variance threshold is 0.0"):
        UtilityJustificationGraphReceipt.model_validate({
            "optimizing_vectors": {"a": 1.0}, 
            "degrading_vectors": {"b": 1.0}, 
            "ensemble_spec": {
                "concurrent_branch_cids": ["branch_12345"], 
                "fusion_function": "weighted_consensus"
            }, 
            "superposition_variance_threshold": 0.0
        })

def test_tensor_poisoning_inf():
