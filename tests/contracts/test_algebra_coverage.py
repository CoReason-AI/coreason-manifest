...
def test_generate_correction_prompt_missing_and_invalid() -> None:
    ...
    try:
        WorkflowManifest(
            ...
            topology=DAGTopologyManifest(manifold_category="dag", nodes={}, edges=[], max_depth=1, max_fan_out=1),
        )
    except ValidationError as e:
        prompt = synthesize_remediation_intent(e, "did:node:faulty1", "fault1")
...
@given(
    ops=st.lists(
        st.builds(
            StateTransmutationIntent, op=st.sampled_from(["test", "add"]), path=st.just("/foo"), value=st.just("bar")
        )
    )
)
def test_apply_state_differential_hyp_add(ops: list[StateTransmutationIntent]) -> None:
    state: dict[str, Any] = {}
...
def test_apply_state_differential_test_fail() -> None:
    ...
    manifest = StateDifferentialManifest(
        ...
        patches=[StateTransmutationIntent(op="test", path="/foo", value="bar")],
    )
...
def test_apply_state_differential_copy() -> None:
    ...
    manifest = StateDifferentialManifest(
        ...
        lamport_timestamp=1,
        vector_clock={"n111111": 1},
        patches=[StateTransmutationIntent(**{"op": "copy", "from": "/foo/bar", "path": "/foo/qux"})],  # type: ignore[arg-type]
    )
...
def test_apply_state_differential_move() -> None:
    ...
    manifest = StateDifferentialManifest(
        ...
        lamport_timestamp=1,
        vector_clock={"n111111": 1},
        patches=[StateTransmutationIntent(**{"op": "move", "from": "/foo/bar", "path": "/foo/qux"})],  # type: ignore[arg-type]
    )
    res = transmute_state_differential(state, manifest)
    assert res["foo"]["qux"] == "baz"
...
def test_apply_state_differential_replace_list() -> None:
    ...
    manifest = StateDifferentialManifest(
        ...
        lamport_timestamp=1,
        vector_clock={"n111111": 1},
        patches=[StateTransmutationIntent(op="replace", path="/foo/1", value=99)],
    )
    res = transmute_state_differential(state, manifest)
    assert res["foo"][1] == 99
...
def test_apply_state_differential_remove_list() -> None:
    ...
    manifest = StateDifferentialManifest(
        ...
        lamport_timestamp=1,
        vector_clock={"n111111": 1},
        patches=[StateTransmutationIntent(op="remove", path="/foo/1")],
    )
    res = transmute_state_differential(state, manifest)
    assert res["foo"] == [1, 3]
...
def test_apply_state_differential_add_list_dash() -> None:
    ...
    manifest = StateDifferentialManifest(
        ...
        lamport_timestamp=1,
        vector_clock={"n111111": 1},
        patches=[StateTransmutationIntent(op="add", path="/foo/-", value=3)],
    )
    res = transmute_state_differential(state, manifest)
...
def test_apply_state_differential_invalid_root() -> None:
    manifest = StateDifferentialManifest(
        ...
        lamport_timestamp=1,
        vector_clock={"n111": 1},
        patches=[StateTransmutationIntent(op="add", path="invalid", value="bar")],
    )
    with pytest.raises(ValueError, match="Patch operation failed"):
...
def test_apply_state_differential_invalid_from_path() -> None:
    manifest = StateDifferentialManifest(
        ...
        lamport_timestamp=1,
        vector_clock={"n111": 1},
        patches=[StateTransmutationIntent(**{"op": "copy", "path": "/foo", "from": "invalid"})],  # type: ignore[arg-type]
    )
    with pytest.raises(ValueError, match="Patch operation failed"):
...
def test_apply_state_differential_out_of_bounds() -> None:
    manifest = StateDifferentialManifest(
        ...
        lamport_timestamp=1,
        vector_clock={"n111": 1},
        patches=[StateTransmutationIntent(op="add", path="/foo/99", value="bar")],
    )
    with pytest.raises(ValueError, match="Patch operation failed"):
...
def test_apply_state_differential_exceptions() -> None:
    ...
    assert (
        cast(
            "Any",
            transmute_state_differential({}, manifest_base([StateTransmutationIntent(op="add", path="", value=1)])),
        )
        == 1
    )
    ...
    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": "b"}, manifest_base([StateTransmutationIntent(op="add", path="/a/b", value=1)])
        )

    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": []}, manifest_base([StateTransmutationIntent(op="add", path="/a/~foo", value=1)])
        )

    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": []}, manifest_base([StateTransmutationIntent(op="add", path="/a/foo/bar", value=1)])
        )

    p = StateTransmutationIntent(**{"op": "copy", "path": "/b", "from": "/a/foo/bar"})  # type: ignore[arg-type]
    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential({"a": [], "b": 1}, manifest_base([p]))

    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": {}}, manifest_base([StateTransmutationIntent(op="add", path="/a/b/c", value=1)])
        )

    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": []},
            manifest_base([StateTransmutationIntent(**{"op": "copy", "path": "/b", "from": "/a/-"})]),  # type: ignore[arg-type]
        )

    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": []},
            manifest_base([StateTransmutationIntent(**{"op": "copy", "path": "/b", "from": "/a/99"})]),  # type: ignore[arg-type]
        )

    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": []},
            manifest_base([StateTransmutationIntent(**{"op": "copy", "path": "/b", "from": "/a/foo"})]),  # type: ignore[arg-type]
        )

    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential({"a": {}}, manifest_base([StateTransmutationIntent(op="remove", path="/a/foo")]))

    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential({"a": []}, manifest_base([StateTransmutationIntent(op="remove", path="/a/-")]))

    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential({"a": []}, manifest_base([StateTransmutationIntent(op="remove", path="/a/99")]))
    ...
    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": []}, manifest_base([StateTransmutationIntent(op="add", path="/a/99", value=1)])
        )

    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(1, manifest_base([StateTransmutationIntent(op="add", path="/a", value=1)]))  # type: ignore[arg-type]

    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": {"b": 1}}, manifest_base([StateTransmutationIntent(op="add", path="/a/b/c/d", value=1)])
        )

    # Hit 369: valid list navigation
    transmute_state_differential(
        {"a": [{"b": 1}]}, manifest_base([StateTransmutationIntent(op="test", path="/a/0/b", value=1)])
    )

    # Hit 371: invalid list navigation
    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": []}, manifest_base([StateTransmutationIntent(op="add", path="/a/foo/b", value=1)])
        )

    # Hit 391: valid from_path list navigation
    p_valid = StateTransmutationIntent(**{"op": "copy", "path": "/c", "from": "/a/0/b"})  # type: ignore[arg-type]
    transmute_state_differential({"a": [{"b": 1}], "c": 0}, manifest_base([p_valid]))

    # Hit 386: from_path missing parent dict key
    p_invalid_key = StateTransmutationIntent(**{"op": "copy", "path": "/c", "from": "/a/foo/bar"})  # type: ignore[arg-type]
    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential({"a": {}}, manifest_base([p_invalid_key]))

    p_invalid = StateTransmutationIntent(**{"op": "copy", "path": "/c", "from": "/a/b/c/d"})  # type: ignore[arg-type]
    with pytest.raises(ValueError, match="Patch operation failed"):
...
def test_apply_state_differential_copy_ops() -> None:
    ...
    res = transmute_state_differential(
        {"a": {"b": 1}},
        manifest_base([StateTransmutationIntent(**{"op": "copy", "path": "/a/c", "from": "/a/b"})]),  # type: ignore[arg-type]
    )
    assert res["a"]["c"] == 1
    ...
    res = transmute_state_differential(
        {"a": [1, 2]},
        manifest_base([StateTransmutationIntent(**{"op": "move", "path": "/a/-", "from": "/a/0"})]),  # type: ignore[arg-type]
    )
    assert res["a"] == [2, 1]

    # Test replace op inside dict
    res = transmute_state_differential(
        {"a": {"b": 1}}, manifest_base([StateTransmutationIntent(op="replace", path="/a/b", value=2)])
    )
    assert res["a"]["b"] == 2
    ...
    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": {"b": 1}},
            manifest_base([StateTransmutationIntent(**{"op": "copy", "path": "/a/b/c", "from": "/a/b"})]),  # type: ignore[arg-type]
        )

    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential({"a": 1}, manifest_base([StateTransmutationIntent(op="copy", path="/b")]))

    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": []}, manifest_base([StateTransmutationIntent(op="replace", path="/a/-", value=1)])
        )

    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": []}, manifest_base([StateTransmutationIntent(op="replace", path="/a/foo", value=1)])
        )
    ...
    transmute_state_differential(
        {"a": [1]},
        manifest_base([StateTransmutationIntent(**{"op": "copy", "path": "/a/-", "from": "/a/0"})]),  # type: ignore[arg-type]
    )

    # copy insert to list (0)
    transmute_state_differential(
        {"a": [1]},
        manifest_base([StateTransmutationIntent(**{"op": "copy", "path": "/a/0", "from": "/a/0"})]),  # type: ignore[arg-type]
    )
    ...
    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": [1]},
            manifest_base([StateTransmutationIntent(**{"op": "copy", "path": "/a/foo", "from": "/a/0"})]),  # type: ignore[arg-type]
        )

    # move insert to same list (from_idx < last_part)
    transmute_state_differential(
        {"a": [1, 2, 3]},
        manifest_base([StateTransmutationIntent(**{"op": "move", "path": "/a/2", "from": "/a/0"})]),  # type: ignore[arg-type]
    )

    # move insert to same list (from_idx >= last_part)
    transmute_state_differential(
        {"a": [1, 2, 3]},
        manifest_base([StateTransmutationIntent(**{"op": "move", "path": "/a/0", "from": "/a/2"})]),  # type: ignore[arg-type]
    )


def test_compute_topology_hash() -> None:
    top = DAGTopologyManifest(manifold_category="dag", nodes={}, edges=[], max_depth=1, max_fan_out=1)
...
def test_apply_state_differential_test_op() -> None:
    ...
    with pytest.raises(ValueError, match="Patch operation failed"):
        transmute_state_differential(
            {"a": 1}, manifest_base([StateTransmutationIntent(op="test", path="", value={"a": 2})])
        )

    res = transmute_state_differential(
        {"a": 1}, manifest_base([StateTransmutationIntent(op="test", path="", value={"a": 1})])
    )
    assert res == {"a": 1}
