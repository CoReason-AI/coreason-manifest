...
def test_generate_correction_prompt_missing_and_invalid():
    ...
    try:
        WorkflowManifest(
            ...
            tenant_id="t1",
            session_id="s1",
            genesis_provenance={"author_identity": "did:node:n1"},
...
def test_apply_state_differential_copy():
    ...
    manifest = StateDifferentialManifest(
        ...
        lamport_timestamp=1,
        vector_clock={"n111111": 1},
        patches=[StateMutationIntent(**{"op": "copy", "from": "/foo/bar", "path": "/foo/qux"})],
    )
...
def test_apply_state_differential_move():
    ...
    manifest = StateDifferentialManifest(
        ...
        lamport_timestamp=1,
        vector_clock={"n111111": 1},
        patches=[StateMutationIntent(**{"op": "move", "from": "/foo/bar", "path": "/foo/qux"})],
    )
    res = apply_state_differential(state, manifest)
    assert res["foo"]["qux"] == "baz"
...
def test_apply_state_differential_invalid_from_path():
    manifest = StateDifferentialManifest(
        ...
        lamport_timestamp=1,
        vector_clock={"n111": 1},
        patches=[StateMutationIntent(**{"op": "copy", "path": "/foo", "from": "invalid"})],
    )
...
def test_apply_state_differential_exceptions():
    ...
    with pytest.raises(ValueError, match="Cannot extract"):
        apply_state_differential(
            {"a": []}, manifest_base([StateMutationIntent(**{"op": "copy", "path": "/b", "from": "/a/-"})])
        )

    with pytest.raises(ValueError, match="Invalid from_path operation: Invalid index"):
        apply_state_differential(
            {"a": []}, manifest_base([StateMutationIntent(**{"op": "copy", "path": "/b", "from": "/a/99"})])
        )

    with pytest.raises(ValueError, match="Invalid from_path operation: Invalid index"):
        apply_state_differential(
            {"a": []}, manifest_base([StateMutationIntent(**{"op": "copy", "path": "/b", "from": "/a/foo"})])
        )
...
def test_apply_state_differential_copy_ops():
    ...
    res = apply_state_differential(
        {"a": {"b": 1}}, manifest_base([StateMutationIntent(**{"op": "copy", "path": "/a/c", "from": "/a/b"})])
    )
    assert res["a"]["c"] == 1

    # Test move op inside list
    res = apply_state_differential(
        {"a": [1, 2]}, manifest_base([StateMutationIntent(**{"op": "move", "path": "/a/-", "from": "/a/0"})])
    )
    assert res["a"] == [2, 1]
    ...
    with pytest.raises(ValueError, match="proper prefix"):
        apply_state_differential(
            {"a": {"b": 1}}, manifest_base([StateMutationIntent(**{"op": "copy", "path": "/a/b/c", "from": "/a/b"})])
        )
    ...
    # copy append to list (-)
    apply_state_differential(
        {"a": [1]}, manifest_base([StateMutationIntent(**{"op": "copy", "path": "/a/-", "from": "/a/0"})])
    )

    # copy insert to list (0)
    apply_state_differential(
        {"a": [1]}, manifest_base([StateMutationIntent(**{"op": "copy", "path": "/a/0", "from": "/a/0"})])
    )

    # copy insert to list invalid index
    with pytest.raises(ValueError, match="Invalid index"):
        apply_state_differential(
            {"a": [1]}, manifest_base([StateMutationIntent(**{"op": "copy", "path": "/a/foo", "from": "/a/0"})])
        )

    # move insert to same list (from_idx < last_part)
    apply_state_differential(
        {"a": [1, 2, 3]}, manifest_base([StateMutationIntent(**{"op": "move", "path": "/a/2", "from": "/a/0"})])
    )

    # move insert to same list (from_idx >= last_part)
    apply_state_differential(
        {"a": [1, 2, 3]}, manifest_base([StateMutationIntent(**{"op": "move", "path": "/a/0", "from": "/a/2"})])
    )
