...
def test_mock_semantics_combinators() -> None:
    ...
    result_allof = factory._generate_schema_data(schema_allof)
    assert isinstance(result_allof, dict)
    assert result_allof["a"] == 1
    assert result_allof["b"] == 2
