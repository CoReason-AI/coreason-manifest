    ...
    return draw(
        st.one_of(
            ...
            st.builds(
                ...
                id=st.text(min_size=1),
                semantic_role=st.text(),
                artifact_uri=st.text(min_size=1),
                maintain_aspect_ratio=st.booleans(),
            ),
        )
    )
...
def test_data_artifact_element() -> None:
    element = DataArtifactElement(
        id="da-1",
        semantic_role="results_bar_chart",
        artifact_uri="urn:data:123",
        maintain_aspect_ratio=True,
    )
    assert element.id == "da-1"
    assert element.element_type == GraphicElementType.DATA_ARTIFACT
