...
def test_init_success(tmp_path, capsys):
    ...
    assert f"Created new agent project in '{project_dir}'" in captured.out or \
...
def test_init_safety_check(tmp_path, capsys):
    ...
    assert f"Error: Directory '{project_name}' is not empty." in captured.out or \
           f"Error: Directory '{project_dir}' is not empty." in captured.out

def test_init_existing_empty_dir(tmp_path, capsys):
