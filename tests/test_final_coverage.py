...
from coreason_manifest.spec.core.flow import FlowMetadata, LinearFlow, GraphFlow, Graph, Edge
...
from coreason_manifest.utils.validator import validate_flow, _validate_kill_switch
...
def test_visualizer_mermaid_conditional() -> None:
    ...
    graph = Graph(
        nodes={"n1": n1, "n2": n2},
        edges=[Edge(from_node="n1", to_node="n2", condition="x>1")],
        entry_point="n1"
    )
    flow = GraphFlow.model_construct(
        kind="GraphFlow", metadata=FlowMetadata(name="t", version="1"),
        graph=graph
    )
...
def test_visualizer_react_flow_presentation() -> None:
    ...
    flow = LinearFlow.model_construct(
        kind="LinearFlow", metadata=FlowMetadata(name="t", version="1"),
        steps=[n1]
    )
