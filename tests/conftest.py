...
def safe_profile() -> CognitiveProfile:
    return CognitiveProfile(role="tester", persona="safe", reasoning=StandardReasoning(model="gpt-4-turbo"), fast_path=None)

@pytest.fixture
def unsafe_profile() -> CognitiveProfile:
    return CognitiveProfile(role="hacker", persona="unsafe", reasoning=ComputerUseReasoning(model="claude-3-5-sonnet"), fast_path=None)

@pytest.fixture
def code_exec_profile() -> CognitiveProfile:
    return CognitiveProfile(role="coder", persona="unsafe_coder", reasoning=CodeExecutionReasoning(model="gpt-4", allow_network=True, timeout_seconds=10), fast_path=None)
