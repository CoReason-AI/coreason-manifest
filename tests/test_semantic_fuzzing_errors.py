...
def test_vector_embedding_dimensionality_mismatch() -> None:
...
def test_action_space_duplicate_tools() -> None:
    tool = {
        "tool_name": "dup_tool",
        "description": "test",
        "input_schema": {},
    ...
    with pytest.raises(ValidationError) as exc_info:
        ActionSpace.model_validate({"action_space_id": "test", "native_tools": [tool, tool], "mcp_servers": []})
