...
def create_agent(tools: Optional[List[ToolType]] = None, requires_auth: bool = False) -> AgentDefinition:
    if tools is None:
        tools = []
    ...
    config = AgentRuntimeConfig(
        llm_config=ModelConfig(model="gpt-4", temperature=0.7, system_prompt="You are a helpful assistant.")
    )

    dependencies = AgentDependencies(tools=tools)

    return AgentDefinition(metadata=metadata, capabilities=[capability], config=config, dependencies=dependencies)
...
def test_governance_domain_restriction_pass() -> None:
    tool = ToolRequirement(uri="https://trusted.com/tool", hash="a" * 64, scopes=[], risk_level=ToolRiskLevel.SAFE)
...
def test_governance_domain_restriction_fail() -> None:
    tool = ToolRequirement(uri="https://untrusted.com/tool", hash="a" * 64, scopes=[], risk_level=ToolRiskLevel.SAFE)
    agent = create_agent(tools=[tool])
    config = GovernanceConfig(allowed_domains=["trusted.com"])
    report = check_compliance(agent, config)
...
def test_governance_risk_level_pass() -> None:
    tool = ToolRequirement(uri="https://example.com/tool", hash="a" * 64, scopes=[], risk_level=ToolRiskLevel.SAFE)
    agent = create_agent(tools=[tool])
...
def test_governance_risk_level_fail() -> None:
    tool = ToolRequirement(uri="https://example.com/tool", hash="a" * 64, scopes=[], risk_level=ToolRiskLevel.CRITICAL)
    agent = create_agent(tools=[tool])
    config = GovernanceConfig(max_risk_level=ToolRiskLevel.STANDARD)
    report = check_compliance(agent, config)
    assert not report.passed
...
def test_governance_inline_tool_ignored() -> None:
    ...
    agent = create_agent(tools=[tool])
    config = GovernanceConfig(allowed_domains=["trusted.com"], max_risk_level=ToolRiskLevel.SAFE)
    report = check_compliance(agent, config)
    assert report.passed
...
def test_governance_multiple_violations() -> None:
    ...
    config = GovernanceConfig(
        allowed_domains=["trusted.com"], max_risk_level=ToolRiskLevel.STANDARD, require_auth_for_critical_tools=True
    )
    report = check_compliance(agent, config)
    assert not report.passed
