...
def test_flow_builder_operational_policy() -> None:
    ...
    assert op.financial is not None
    assert op.financial.max_cost_usd == 10.0

    assert op.compute is not None
    ...
    assert flow_partial.governance is not None
    assert flow_partial.governance.operational_policy is not None
    assert flow_partial.governance.operational_policy.financial is not None
    ...
    assert flow_partial_compute.governance is not None
    assert flow_partial_compute.governance.operational_policy is not None
    assert flow_partial_compute.governance.operational_policy.financial is None
    assert flow_partial_compute.governance.operational_policy.compute is not None
    ...
    assert flow_partial_data.governance is not None
    assert flow_partial_data.governance.operational_policy is not None
    ...
    assert flow_partial_data.governance.operational_policy.data is not None
    assert flow_partial_data.governance.operational_policy.data.max_rows_per_query == 10


def test_flow_builder_circuit_breaker() -> None:
    flow = NewLinearFlow("test-flow")
...
def test_flow_builder_add_agent_ref() -> None:
    flow = NewLinearFlow("test-flow")
...
def test_flow_builder_add_shadow_node() -> None:
    flow = NewLinearFlow("test-flow")
...
def test_flow_builder_add_inspector() -> None:
    flow = NewLinearFlow("test-flow")
...
def test_new_linear_flow_methods() -> None:
    flow = NewLinearFlow("test-flow")
...
def test_new_graph_flow_methods() -> None:
...
def test_agent_builder_build_validation() -> None:
    builder = AgentBuilder("agent-1")
    # Missing identity
...
def test_create_resilience_coverage() -> None:
    # Test retry
...
def test_agent_builder_operational_policy() -> None:
    ...
    # Test partials to cover all branches

    # Only financial
    builder.with_operational_policy(max_cost_usd=5.0)
    assert builder.operational_policy is not None
    assert builder.operational_policy.financial is not None
    assert builder.operational_policy.financial.max_cost_usd == 5.0
    assert builder.operational_policy.compute is None
    assert builder.operational_policy.data is None

    # Only compute
    builder = AgentBuilder("agent-op-compute")
    builder.with_operational_policy(max_steps=10)
    assert builder.operational_policy is not None
    assert builder.operational_policy.financial is None
    assert builder.operational_policy.compute is not None
    assert builder.operational_policy.compute.max_cognitive_steps == 10
    assert builder.operational_policy.data is None

    # Only data
    builder = AgentBuilder("agent-op-data")
    builder.with_operational_policy(max_rows_per_query=10)
    assert builder.operational_policy is not None
    assert builder.operational_policy.financial is None
    assert builder.operational_policy.compute is None
...
def test_flow_builder_set_governance() -> None:
    ...
    gov = Governance()
    flow.set_governance(gov)
    assert flow.governance == gov
