...
def test_explicit_keys_and_list_limits() -> None:
    ...
    with pytest.raises(ValidationError) as exc:
        BoundedJSONRPCIntent.model_validate({"jsonrpc": "2.0", "method": "test", "params": many_keys_dict, "id": 1})
    assert "Dictionary exceeds maximum of 100 keys" in str(exc.value)

    # Key too long (>1000)
    long_key_dict = {"K" * 1005: "v"}
    with pytest.raises(ValidationError) as exc:
        BoundedJSONRPCIntent.model_validate({"jsonrpc": "2.0", "method": "test", "params": long_key_dict, "id": 1})
    assert "Dictionary key exceeds maximum length of 1000" in str(exc.value)

    # Null params coverage
    BoundedJSONRPCIntent.model_validate({"jsonrpc": "2.0", "method": "test", "params": None, "id": 1})

    # Very long string in list
    with pytest.raises(ValidationError) as exc:
        BoundedJSONRPCIntent.model_validate(
    ...
    # Invalid params type
    with pytest.raises(ValidationError) as exc:
        BoundedJSONRPCIntent.model_validate({"jsonrpc": "2.0", "method": "test", "params": "not a dict", "id": 1})
