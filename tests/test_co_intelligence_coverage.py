...
def test_escalation_criteria_validation_syntax_error():
...
def test_escalation_criteria_validation_security_violation_call():
    """Test that function calls are forbidden in condition."""
    with pytest.raises(ValidationError) as excinfo:
        EscalationCriteria(
            condition="print('evil')",
            role="supervisor"
        )
    assert "Security Violation" in str(excinfo.value)


def test_steering_config_empty_allowed_targets():
    """
