...
def test_langchain_adapter() -> None:
    ...
    expected_nodes = [
        {"id": "agent1", "type": "agent", "metadata": {}},
        {"id": "agent2", "type": "agent", "metadata": {}},
    ]
    # Sort by ID to ensure consistent comparison
    lc_nodes = sorted(lc_graph["nodes"], key=lambda x: str(x["id"]))
    expected_nodes = sorted(expected_nodes, key=lambda x: str(x["id"]))

    assert lc_nodes == expected_nodes
