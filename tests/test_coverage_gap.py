...
from coreason_manifest.builder import NewGraphFlow, AgentNode, CognitiveProfile
from coreason_manifest.utils.diff import compare_manifests, ChangeCategory, _diff_switch_node, _diff_agent_node
...
from coreason_manifest.spec.core.nodes import SwitchNode, InspectorNode
from coreason_manifest.spec.core.tools import ToolPack
from coreason_manifest.spec.core.engines import StandardReasoning, Supervision
from coreason_manifest.spec.core.governance import Governance
from pydantic import BaseModel

# --- Builder Tests ---

def test_builder_add_computer_use():
...
def test_builder_validate_flow_errors():
...
def test_builder_add_emergence_inspector():
    ...
    assert node.judge_model == "gpt-4"

def test_builder_connect_missing_source():
...
def test_builder_connect_missing_target():
    builder = NewGraphFlow("Connect", "1.0", "Desc")
...
def test_diff_agent_node_tools():
...
def test_diff_node_type_change():
...
def test_diff_node_supervision_change():
...
def test_diff_node_metadata_change():
...
def test_hashing_recursive():
    data = {"a": {"b": [1.0, 2]}}
    assert canonicalize(data) == b'{"a":{"b":[1,2]}}'

def test_verify_chain_missing_hash():
    # Entry without stored hash
    chain = [{"data": "foo"}]
    assert not verify_chain(chain)

def test_verify_chain_bad_hash():
...
def test_verify_chain_object_access():
    class Entry:
...
def test_secure_loader_jail_escape_initial(tmp_path):
    root = tmp_path / "jail"
    root.mkdir()
...
def test_secure_loader_different_drive(tmp_path):
    root = tmp_path / "jail"
    root.mkdir()
    loader = SecureLoader(root)
...
def test_secure_loader_resolve_ref_different_drive(tmp_path):
    root = tmp_path / "jail"
    root.mkdir()
    loader = SecureLoader(root)
...
def test_secure_loader_type_error(tmp_path):
    root = tmp_path / "jail"
    root.mkdir()
    loader = SecureLoader(root)
...
def test_secure_loader_recursion(tmp_path):
    root = tmp_path / "jail"
    root.mkdir()
...
def test_secure_loader_file_not_found(tmp_path):
    root = tmp_path / "jail"
    root.mkdir()
    loader = SecureLoader(root)
...
def test_secure_loader_scalar_content(tmp_path):
    root = tmp_path / "jail"
    root.mkdir()
    loader = SecureLoader(root)
...
def test_secure_loader_invalid_ref_type(tmp_path):
    root = tmp_path / "jail"
    root.mkdir()
    loader = SecureLoader(root)
