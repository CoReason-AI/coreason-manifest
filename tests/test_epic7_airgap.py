...
def test_airgap_socket_connect(tmp_path: Path) -> None:
    ...
    with sandbox_context(jail_root):
        ...
        with pytest.raises(
            SecurityViolationError, match=r"Network access blocked in sandbox: socket\.connect"
        ):
...
def test_airgap_urllib_request(tmp_path: Path) -> None:
    ...
    with sandbox_context(jail_root), pytest.raises(
        SecurityViolationError, match=r"Network access blocked in sandbox: urllib\.Request"
    ):
