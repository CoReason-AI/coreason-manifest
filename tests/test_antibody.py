...
def test_antibody_strict_float_failure() -> None:
    ...
    Pydantic then validates the field against 'float', which fails because DataAnomaly is not float.
    ...
    # Pydantic standard validation error because DataAnomaly is not a float
...
def test_antibody_any_field_mutation() -> None:
    ...
    Antibody converts it to DataAnomaly, and Pydantic accepts it.
...
def test_antibody_list_mutation() -> None:
    ...
    assert isinstance(model.items[1], DataAnomaly)
